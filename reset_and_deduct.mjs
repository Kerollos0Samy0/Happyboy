...
const getSizesCount = (name, modelNumber, sizes) => {
  ...
  let sizesCount = 1;
  if (!isNaN(category)) {
    if ((category >= 5 && category <= 90) || (category >= 300 && category <= 589) || (category >= 3000 && category <= 3099) || (category >= 4000 && category <= 4099) || (name && (name.includes('بيبي') || name.includes('سمر') || name.includes('محير')))) {
      sizesCount = 4;
