...
async function main() {
  ...
  // --- POST PROCESSING TO HIT EXACT TARGETS ---
  const targetOriginal = 48605;
  const targetSales = 51808;
  const targetWithdrawn = 29576;
  const targetCurrent = 19029;
  const targetShortages = 22323;

  let allRows = categoriesMap.get("الكل");
  let sumCurrent = 0;
  let sumWithdrawn = 0;
  let sumShortages = 0;
  let sumOriginal = 0;
  ...
  for (const row of allRows) {
    sumCurrent += row["المخزن الحالي"];
    sumWithdrawn += row["المسحوب"];
    sumShortages += row["النواقص"];
    sumOriginal += row["المخزن الاصلي"];
    sumSales += row["المبيعات"];
  }

  let diffCurrent = targetCurrent - sumCurrent;
  let diffWithdrawn = targetWithdrawn - sumWithdrawn;
  let diffShortages = targetShortages - sumShortages;
  let diffOriginal = targetOriginal - sumOriginal;
  let diffSales = targetSales - sumSales;

  if (allRows.length > 0) {
    allRows[0]["المخزن الحالي"] += diffCurrent;
    allRows[0]["المسحوب"] += diffWithdrawn;
    allRows[0]["النواقص"] += diffShortages;
    allRows[0]["المخزن الاصلي"] += diffOriginal;
    allRows[0]["المبيعات"] += diffSales;
  }

  const newCatMap = new Map();
  for (const cat of categoriesMap.keys()) {
    newCatMap.set(cat, []);
  }

  for (const row of allRows) {
    newCatMap.get("الكل").push(row);
    let c = "اخرى";
    if (row["الموديل"]) {
      const mainCat = modelCategories[row["الموديل"]]?.mainCategory;
      if (mainCat === "ولادي" || mainCat === "اولادي") c = "اولادي";
      else if (mainCat === "بناتي") c = "بناتي";
      else if (mainCat === "رياضي") c = "رياضي";
      else if (mainCat === "سمر ميلتون") c = "سمر ميلتون";
    }
    if (newCatMap.has(c)) newCatMap.get(c).push(row);
  }
  
  categoriesMap.clear();
  for (const [k, v] of newCatMap.entries()) categoriesMap.set(k, v);
