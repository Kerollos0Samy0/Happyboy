...
async function run() {
  ...
  // stockMap: model -> { byBarcode: { barcode: qty }, byColor: { normColor: qty } }
  const stockMap = {};
  for (const row of excelData) {
      const modelCode = row['كود الموديل'];
      const barcode = row['الباركود'];
      const colorNameRaw = row['التصنيف']; // typically "ترنج اولادى (منت)" or something similar
      const qty = row['عدد القطع'];
      
      if (modelCode !== undefined && qty !== undefined) {
          const m = String(modelCode).trim();
          if (!stockMap[m]) stockMap[m] = { byBarcode: {}, byColor: {} };
          
          let parsedQty = parseInt(qty, 10) || 0;
          
          if (barcode !== undefined) {
              const b = String(barcode).trim();
              stockMap[m].byBarcode[b] = (stockMap[m].byBarcode[b] || 0) + parsedQty;
          }
          
          if (colorNameRaw !== undefined) {
              // Extract color from parentheses e.g. "ترنج اولادى (منت)" -> "منت"
              const match = String(colorNameRaw).match(/\((.*?)\)/);
              let c = match ? match[1] : String(colorNameRaw);
              const nc = normalize(c);
              stockMap[m].byColor[nc] = (stockMap[m].byColor[nc] || 0) + parsedQty;
          }
      }
  }
  ...
  for (const order of allOrders) {
      ...
      let canFulfill = true;
      const deductions = []; // Keep track of what we need to deduct if successful
      const shortages = []; // Track what exactly is missing
      ...
      for (const item of order.items) {
          ...
          if (stockMap[m]) {
              if (b && stockMap[m].byBarcode[b] !== undefined) {
                  available = stockMap[m].byBarcode[b];
                  matchedBy = { type: 'barcode', key: b };
              } else if (stockMap[m].byColor[nc] !== undefined) {
                  available = stockMap[m].byColor[nc];
                  matchedBy = { type: 'color', key: nc };
              }
          }
          
          // Check previous deductions in THIS order so we don't double count available
          let alreadyDeductedThisOrder = 0;
          for (const d of deductions) {
              if (d.model === m && d.matchedBy && matchedBy && d.matchedBy.type === matchedBy.type && d.matchedBy.key === matchedBy.key) {
                  alreadyDeductedThisOrder += d.qty;
              }
          }
          
          if (available - alreadyDeductedThisOrder >= qty) {
              deductions.push({ model: m, qty: qty, matchedBy });
          } else {
              canFulfill = false;
              shortages.push(`موديل ${m} (لون: ${c}, باركود: ${b}) - المطلوب: ${qty}, المتاح: ${Math.max(0, available - alreadyDeductedThisOrder)}`);
          }
      }
      
      if (canFulfill) {
          // Commit deductions to stockMap
          for (const d of deductions) {
              if (d.matchedBy) {
                  if (d.matchedBy.type === 'barcode') {
                      stockMap[d.model].byBarcode[d.matchedBy.key] -= d.qty;
                  } else {
                      stockMap[d.model].byColor[d.matchedBy.key] -= d.qty;
                  }
              }
          }
  ...
  writeFileSync("orders_status_report.md", md, "utf-8");
