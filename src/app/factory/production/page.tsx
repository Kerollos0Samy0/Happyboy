...
const STAGES = [
  { id: 1, name: "??? ???????", icon: Microscope, color: 'bg-gray-100 text-gray-700' },
  { id: 2, name: "????? ????", icon: ShoppingCart, color: 'bg-blue-50 text-blue-600' },
  { id: 3, name: "???? ????", icon: Package, color: 'bg-blue-100 text-blue-700' },
  { id: 4, name: "??? ????", icon: Scissors, color: 'bg-orange-100 text-orange-700' },
  { id: 5, name: "??? ?????", icon: Layers, color: 'bg-yellow-100 text-yellow-700' },
  { id: 6, name: "??? ??????? - ????", icon: Printer, color: 'bg-purple-100 text-purple-700' },
  { id: 7, name: "??? ???? ????????", icon: Wand2, color: 'bg-indigo-100 text-indigo-700' },
  { id: 8, name: "??? ?????", icon: Settings, color: 'bg-pink-100 text-pink-700' },
  { id: 9, name: "??? ???????", icon: Copy, color: 'bg-rose-100 text-rose-700' },
  { id: 10, name: "??? ?????", icon: Shirt, color: 'bg-green-100 text-green-700' },
  { id: 11, name: "??? ???????", icon: CheckCircle, color: 'bg-teal-100 text-teal-700' },
  { id: 12, name: "??? ???????", icon: Wind, color: 'bg-cyan-100 text-cyan-700' },
  { id: 13, name: "??????? ????????", icon: Archive, color: 'bg-emerald-100 text-emerald-700' },
  { id: 14, name: "???? ?????????", icon: Store, color: 'bg-gray-100 text-gray-800' }
];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <h2 className="text-lg font-bold mb-6 border-b pb-2 text-gray-800">?? ??? ???????? (14 ???) ????? ???????</h2>
                  ...
                  <div key={dept.id} className={`border rounded-xl p-4 flex flex-col items-center justify-center text-center transition relative ${hasWork ? 'bg-white shadow-md border-blue-200' : 'bg-gray-50 opacity-70 border-gray-100'}`}>
                    ...
                    {loading ? (
                      ...
                      <div className="bg-blue-50 w-full rounded p-1.5 border border-blue-100">
                        <p className="text-xs font-bold text-blue-700">{deptStats.orders} ?????</p>
                        <p className="text-[10px] font-bold text-blue-500">({deptStats.pieces.toLocaleString()} ????)</p>
                      </div>
                    ) : (
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <h2 className="text-lg font-bold mb-4 border-b pb-2 text-gray-800">??????? ?????</h2>
            <div className="space-y-3">
              <Link href="/factory/dashboard" className="w-full text-right p-3 bg-gray-50 hover:bg-blue-50 rounded-lg transition border border-transparent hover:border-blue-100 flex items-center justify-between group">
                <span className="font-medium text-gray-700 group-hover:text-blue-700">???? ??????? (Kanban)</span>
              ...
              <Link href="/factory/inventory" className="w-full text-right p-3 bg-gray-50 hover:bg-green-50 rounded-lg transition border border-transparent hover:border-green-100 flex items-center justify-between group">
                <span className="font-medium text-gray-700 group-hover:text-green-700">??? ???????</span>
