...
async function check() {
  ...
  snap.forEach(doc => {
    ...
    if (!o.isDeleted && o.items) {
      o.items.forEach(i => {
        const cb = (i.colorBarcode || "").toString().trim();
        if (!cb) {
          const qty = i.quantity || 1;
          const pieces = i.isSeri ? getSizesCount(i.name || '', i.modelNumber || '', i.sizes) * qty : qty;
          missing += pieces;
          missingItems.push(i);
        }
      });
  ...
  console.log("Sold pieces without barcode:", missing);
  console.log(missingItems.map(i=>`${i.modelNumber} - ${i.name} (${i.selectedColor})`));
  process.exit(0);
}
check();
