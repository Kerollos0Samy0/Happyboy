...
async function main() {
  ...
  for (const row of storeData) {
    let cb = (row["Column1"] || "").toString().trim();
    if (cb) {
      const qty = Number(row["قطعة"]) || 0;
      const cat = (row["type of item"] || "").toString().trim();
      const model = (row["Code"] || "").toString().trim();
      const colorName = ""; // The user said "حط انت الالوان من عندك" meaning we pull it from productsMap which happens below
  ...
  const outFile = "تقرير_المخزن_النهائي_محدث.xlsx";
